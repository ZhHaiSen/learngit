...
{
	P->next = NULL;   //初始化，头节点的指针指向空，防止野指针造成的危害
	return SUCCESS;
}
...
Status InsertUser(User *P, User *p)   //借阅者
{
	User *t;
	for (t = P; t->next != NULL; t = t->next)   //获取指向最后一个节点的指针
		;
	p->next = NULL;   //新节点指向空，防止新节点成为最后一个节点后，它的指针造成危害
	t->next = p;   //q作为最后一个节点插入到使用者链表中
	return SUCCESS;
}
