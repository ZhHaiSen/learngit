...
Status reservebook(Book *L)
{
	...
	char booknumber[50];   //暂存书号
	Book *l;   //遍历书籍链表
	Borrow *q;   //遍历所要借阅书籍的借阅者链表
	printf("\n\t\t以下是预约借书功能,请按照提示填写相关信息");   //填写借阅证号和所要借阅的书籍的书号
	printf("\t\t _________________ \n");
	printf("\t\t|                 |\n");
	printf("\t\t|    借阅证号     |\n");
	printf("\t\t|_________________|");
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				{
					systemtime();   //log文件
					fprintf(fp2, "%d-%d-%d %d:%d:%d %s预约借书%s", YEAR, MON, DAY, HOUR, MIN, SEC, card, l->ISBN);
					putc('\n', fp2);
					//有书籍来源
					if (l->able_borrow > 0 && LOOP == 0)  
					{
						(l->able_borrow)--;
						printf("\t\t预约成功,请与%d年%d月%d日前来借书\n");
						return SUCCESS;
					}
					else   
					{
...
void allinformation(Book *L)
{
	...
	l = (Book *)malloc(sizeof(Book));
	q = (Borrow *)malloc(sizeof(Borrow));
	char booknumber[50];   //暂存输入的书号
	printf("\t\t请输入你想要查阅的书籍的书号:");
	scanf_s("%s", booknumber, 50);
	for (l = L->next; l != NULL; l = l->next)   //遍历书籍链表
	{
		if (strcmp(l->ISBN, booknumber) == 0)
	...
	if (LOOP == 0)   //不存在该书号对应的书籍
		printf("\t\t图书馆中不存在该书号所对应的书籍\n");
	else   //找到该书号对应的书籍
	{
		systemtime();   //log文件
		fprintf(fp2, "%d-%d-%d %d:%d:%d 查看书籍%s信息", YEAR, MON, DAY, HOUR, MIN, SEC, l->ISBN);
		putc('\n', fp2);
		q = l->borrow;
